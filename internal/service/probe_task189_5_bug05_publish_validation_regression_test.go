...
func TestTask189Bug05_UnvalidatedPackageCannotBePublished(t *testing.T) {
	...
	p, err := packages.Build("unvalidated")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := packages.Publish(p.ID); err == nil {
		panic("unvalidated package was published")
	}
	stored, err := packages.Get(p.ID)
	if err != nil {
		t.Fatal(err)
	}
	if stored.Status == model.PkgPublished {
		panic("package status changed to published despite missing validation")
	}
}
